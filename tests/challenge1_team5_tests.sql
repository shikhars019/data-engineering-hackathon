...
SELECT test.check('C5.02', 'core', '1', $n$Claim status only ever uses the three agreed codes$n$, $e$0$e$,
    $q$SELECT count(*)::text FROM clean.claim WHERE claim_status IS NULL OR claim_status NOT IN ('ACTIVE','SUSPENDED','CLOSED')$q$,
    $h$Anything you did not map will show up here.$h$);
SELECT test.check('C5.03', 'core', '1', $n$978 claims are ACTIVE$n$, $e$978$e$,
    $q$SELECT count(*)::text FROM clean.claim WHERE claim_status = 'ACTIVE'$q$,
    $h$'A', 'LIVE', 'In Payment' and 'INPAY' all mean ACTIVE.$h$);
SELECT test.check('C5.04', 'core', '1', $n$153 claims are CLOSED$n$, $e$153$e$,
    $q$SELECT count(*)::text FROM clean.claim WHERE claim_status = 'CLOSED'$q$,
    $h$'C', 'Ceased' and 'Terminated' all mean CLOSED.$h$);
...
SELECT test.check('C5.06', 'core', '2', $n$642 claims are New State Pension$n$, $e$642$e$,
    $q$SELECT count(*)::text FROM clean.claim WHERE pension_type = 'NEW_STATE_PENSION'$q$,
    $h$'nSP', 'NEW' and 'Single Tier' all mean the same thing.$h$);
...
SELECT test.check('C5.09', 'core', '6', $n$Methods missing from ref.code_mapping became UNKNOWN$n$, $e$1600$e$,
...
SELECT test.check('C5.B1', 'bonus', NULL, $n$69 claims are SUSPENDED$n$, $e$69$e$,
    ...
    $h$Bonus A. 'SUSP', 'S' and 'On Hold' all mean SUSPENDED.$h$);
SELECT test.check('C5.B2', 'bonus', NULL, $n$713 claims are paid four-weekly$n$, $e$713$e$,
    ...
    $h$Bonus B. '4W', '4', 'FOUR WEEKLY' and '4-Weekly' are the same.$h$);
