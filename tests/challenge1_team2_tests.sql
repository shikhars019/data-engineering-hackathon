...
SELECT test.check('C2.06', 'core', '4', $n$A LEGACY_CSV date of death (07/03/2024) was read correctly$n$, $e$2024-03-07$e$,
    $q$SELECT coalesce(to_char(date_of_death, 'YYYY-MM-DD'), '<missing>') FROM clean.claimant WHERE nino = 'QQ680878A'$q$,
    $h$Deaths are written in the same three formats as the birthdays.$h$);
...
SELECT test.check('C2.B3', 'bonus', NULL, $n$Nobody claimed their pension before turning 66$n$, $e$0 too early, out of 1116 checked$e$,
    $q$SELECT count(*) FILTER ( WHERE c.claim_start_date < (p.date_of_birth + INTERVAL '66 years') )::text || ' too early, out of ' || count(*)::text || ' checked' FROM clean.claim c JOIN clean.claimant p ON p.nino = c.nino$q$,
    $h$Bonus B. Needs Bonus A finished first - an empty table checks nothing.$h$);
